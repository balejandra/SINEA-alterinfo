                    ...
                    if ($obligatorio) {
                        for ($x = 0; $x < count($obligatorio); $x++) {
                            ...
                            if ($pr <= $obligatorio[$x]->cantidad_comparacion) {
                                // dd('aq');
                                if ($obligatorio[$x]->nombre_certificado == 'CERTIFICADO NACIONAL DE SEGURIDAD RADIOTELEFONICA') {
                                    $val2 = true;
                                    $nroCorrelativos["certificadoRadio"] = "NO REQUIERE";
                                    $fechavenc["fecha_vencimientocert"] = "NO REQUIERE";
                                } elseif ($obligatorio[$x]->nombre_certificado == 'ASIGNACIÓN DE NÚMERO ISMM') {
                                    // dd('aqui');
                                    $val3 = true;
    ...
    public function permissionCreateStepTwo(Request $request)
    {
        ...
        if (count($mando) == 0) {
            ...
            if ($matricula == "") {
                ...
                $matriculaActual = explode('-', $matricula);
            }
            Flash::error('No se ha podido comparar las especificaciones de la embarcación (' . $matricula . ') respecto a la tabla de mandos actual, comuniquese con el administrador del sistema.');
            return view('zarpes.permiso_zarpe.nacional.create-step-two')
                ->with('paso', 2)
                ->with('titulo', $this->titulo)
                ->with('siglas', $siglas)
                ->with('matriculaActual', $matriculaActual);
        } else {
    ...
    public function validacionMarino(Request $request)
    {
        ...
        if ($capitanExiste) { // SE INTENTA INTRODUCIR OTRO MARINO COMO CAPITAN YA CARGADO UNO
            ...
            if ($mergedData->isEmpty()) {
                ...
                if (is_array($tripulantes)) {
                    for ($i = 0, $iMax = count($tripulantes); $i < $iMax; $i++) {
                        $indice = array_search($cedula, $tripulantes[$i], false);
            ...
            $return = [$tripulantes, $vj, $indice, $ultimoRegistroMarino, $validation['pasajerosRestantes'], $validation['cant_pasajeros'], $validation['cantPassAbordo'], $infoSaime, $trip, $mergedData];
            echo json_encode($return);
        }
    }
